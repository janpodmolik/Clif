...
struct WeeklyUsageStats: Codable, Equatable, UsageStatsProtocol {
    ...
    init(days: [DailyUsageStat], previousWeekTotal: Int? = nil) {
        self.days = days
    ...
    /// Empty stats
    static func empty() -> WeeklyUsageStats {
        WeeklyUsageStats(days: [])
    }

    /// Creates mock data for preview/debug purposes.
    static func mock() -> WeeklyUsageStats {
        let mockPetId = UUID()
        let days = DailyUsageStat.mockList(petId: mockPetId, days: 7)
        let currentTotal = days.reduce(0) { $0 + $1.totalMinutes }
        let previousWeekTotal = Int(Double(currentTotal) * 1.15)
        return WeeklyUsageStats(days: days, previousWeekTotal: previousWeekTotal)
    }
}
